...
class SparseRetrieval:
    ...
    def get_sparse_embedding(self) -> NoReturn:
        ...
        # Pickle을 저장합니다.
        bm25plus_name = f"bm25plus.bin"
        bm25plus_path = os.path.join(self.data_path, bm25plus_name)

        if os.path.isfile(bm25plus_path):
            with open(bm25plus_path, "rb") as file:
            ...
            with open(bm25plus_path, "wb") as file:
    ...
    def build_faiss(self, num_clusters=64) -> NoReturn:
        """
        Summary:
            BM25+는 FAISS를 지원하지 않습니다.
            BM25+는 sparse scoring 방식이므로 dense vector indexing이 필요하지 않습니다.
        """
    ...
    def get_relevant_doc(self, query: str, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        Note:
            BM25+를 사용하여 관련 문서를 검색합니다.
        """

        with timer("tokenize query"):
            tokenized_query = self.tokenize_fn(query)

        with timer("query bm25plus search"):
            doc_scores = self.bm25plus.get_scores(tokenized_query)
    ...
    def get_relevant_doc_bulk(
        ...
        for query in queries:
            tokenized_query = self.tokenize_fn(query)
            scores = self.bm25plus.get_scores(tokenized_query)
